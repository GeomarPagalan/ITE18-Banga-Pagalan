<div class="card rounded-1">
    <div class="card-body">
        <div class="">  
            ...
            <form action="" class="h-50" id="transaction-form">
                <div class="w-100 h-50 mx-0 row row-cols-2 bg-dark">
                    <div class="col-7 h-50 pb-2 d-flex flex-column">
                        <div>
                            <h3 class="text-light">Select product below</h3>
                        </div>
                        <div class="flex-grow-1 d-flex flex-column bg-light bg-opacity-50">
                            ...
                            <div>
                                <table class="table table-hover bg-light mb-1 text-left">
                                    <colgroup>
                                        <col width="20%">
                                        <col width="20%">
                                        <col width="25%">
                            ...
                            <div class="flex-grow-1" >
                                <div class="" style="height:60vh !important ">
                                    <table class="table table-hover bg-light" id="plist">
                                        <colgroup>
                                            <col width="20%">
                                            <col width="20%">
                                            <col width="25%">
                                        ...
                                        $sql = "SELECT p.*,c.name as cname FROM `product_list` p 
                                                ...
                                                where p.status = 1 and p.delete_flag = 0 order by p.`name` asc";
                                        $qry = $conn->query($sql);
                                        while($row = $qry->fetch_assoc()):
                                            $stock_in = $conn->query("SELECT sum(quantity) as `total` 
                                                        FROM `stock_list` where unix_timestamp(`expiry_date`) 
                                                        >= unix_timestamp(CURRENT_TIMESTAMP) and product_id = '{$row['product_id']}' ")
                                                        ->fetch_assoc()['total'];
                                            $stock_out = $conn->query("SELECT sum(quantity) as `total` FROM `transaction_items` 
                                                        where product_id = '{$row['product_id']}' ")->fetch_assoc()['total'];
                                            $stock_in = $stock_in > 0 ? $stock_in : 0;
                                            $stock_out = $stock_out > 0 ? $stock_out : 0;
                                            $qty = $stock_in-$stock_out;
                                            $qty = $qty > 0 ? $qty : 0;
                                        ?>
                                        <tr class="item <?php echo $qty < 50? "bg-info bg-opacity-25":'' ?>" data-id="<?php echo $row['product_id'] ?>">
                                ...
                                <div class="h-100 d-flex w-100 flex-column">
                                    ...
                                    <div>
                                        <table class="table table-hover table-bordered bg-light m-0">
                                            ...
                                            <thead>
                                                <th class="py-0 px-1 text-center">Quantity</th>
                                    ...
                                    <div class="pt-1">
                                        ...
                                        <div class="w-100 d-flex pb-1 justify-content-end">
                                            <button class="btn btn-sm btn-primary pe-2 fs-6 fw-bolder" id="transaction-save-btn" type="button">Save Transaction</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
